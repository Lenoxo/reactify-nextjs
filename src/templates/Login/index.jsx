...
function Login() {
  ...
  const handleLogin = (event) => {
    ...
    const isLoggedIn = userSavedData && userSavedData.email === loginData.email && userSavedData.password === loginData.password;
  ...
  return (
    <Layout>
      ...
      <form onSubmit={handleLogin} className="flex flex-col text-sm items-start border rounded-lg mt-4 border-inherit space-y-4 p-4 dark:bg-zinc-800 dark:text-white dark:border-inherit">
        ...
        <Link to="/sign-up" className="p-4 font-semibold bg-inherit text-inherit w-full rounded-lg border border-black dark:border-inherit disabled:text-zinc-300 disabled:border-zinc-300 text-center">
          Sign Up
        </Link>
      </form>
    </Layout>
